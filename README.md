...
- **Collector** — every completed task generates a *Forretress* with weighted random stats (size, weight, luster, strength — bell-curved toward average), a 1/4096 shiny chance and a 1/100 shadow chance. Browse them in the **Collection** tab — tap one for a full-screen trading-card view (holo frame for shinies, dark frame for shadows). The art is loaded from `public/art/forretress-{normal,shiny,shadow}.png` (hand-drawn, transparent background — see `public/art/README.txt`); any missing file falls back to the built-in procedural SVG.
